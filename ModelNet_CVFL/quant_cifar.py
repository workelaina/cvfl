...
for i in range(num_clients+1):
    ...
    if os.path.exists(PATH):
        ...
        losses = pickle.load(open(f'./loss{suffix}.pkl','rb'))
        accs_train = pickle.load(open(f'./accs_train{suffix}.pkl','rb'))
        accs_test = pickle.load(open(f'./accs_test{suffix}.pkl','rb'))
...
server_model_comp = resnet_top(pretrained=args.pretrained, 
                        num_classes=len(classes), num_clients=num_clients)
